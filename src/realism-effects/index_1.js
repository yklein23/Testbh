...
import { DataTexture, RGBAFormat, FloatType, ShaderChunk, ShaderLib, UniformsUtils, WebGLMultipleRenderTargets, ShaderMaterial, GLSL3, NoBlending, Uniform, Vector2, Matrix4, Vector3, Clock, Quaternion, LinearFilter, HalfFloatType, SRGBColorSpace, FramebufferTexture, TextureLoader, NearestFilter, RepeatWrapping, NoColorSpace, WebGLRenderTarget, ClampToEdgeWrapping, LinearMipMapLinearFilter, EquirectangularReflectionMapping, Color, Matrix3, TangentSpaceNormalMap, RedFormat, Source, Texture, NoToneMapping, PerspectiveCamera, DepthTexture } from 'three';
...
class TemporalReprojectMaterial extends ShaderMaterial {
  constructor(textureCount = 1, temporalReprojectCustomComposeShader = "") {
    let finalFragmentShader = fragmentShader$4.replace("#include <reproject>", reproject).replace("#include <gbuffer_packing>", gbuffer_packing);
...
class TemporalReprojectPass extends Pass {
  ...
  dispose() {
    ...
    (_this$copyPass = this.copyPass) == null ? void 0 : _this$copyPass.dispose();
    this.fullscreenMaterial.dispose();
  }

  setSize(width, height) {
    var _this$copyPass2, _this$framebufferText;

    this.renderTarget.setSize(width, height);
    (_this$copyPass2 = this.copyPass) == null ? void 0 : _this$copyPass2.setSize(width, height);
  ...
  render(renderer) {
    ...
    for (let i = 0; i < this.textureCount; i++) {
      ...
      if (this.copyPass) {
        ...
        copyAccumulatedTexture = this.renderTarget.texture.length > 1 ? this.copyPass.renderTarget.texture[i] : this.framebufferTexture;
      }
    ...
    if (this.renderTarget.texture.length > 1) {
      ...
      (_this$copyPass3 = this.copyPass) == null ? void 0 : _this$copyPass3.render(renderer);
    } else {
...
class SVGF {
  constructor(scene, camera, velocityDepthNormalPass, textureCount = 1, options = {}) {
    this.svgfTemporalReprojectPass = new TemporalReprojectPass(scene, camera, velocityDepthNormalPass, textureCount, { ...options,
      fullAccumulate: true,
      logTransform: true,
      copyTextures: false
    });
  ...
  setJitteredGBuffers(depthTexture, normalTexture, {
    useRoughnessInAlphaChannel = false
...
var ssgi_compose = "#define GLSLIFY 1\nuniform sampler2D inputTexture;uniform sampler2D sceneTexture;uniform sampler2D depthTexture;uniform int toneMapping;\n#include <tonemapping_pars_fragment>\nvoid mainImage(const in vec4 inputColor,const in vec2 uv,out vec4 outputColor){vec4 depthTexel=textureLod(depthTexture,uv,0.);vec3 ssgiClr;if(depthTexel.r==1.0){ssgiClr=textureLod(sceneTexture,uv,0.).rgb;}else{ssgiClr=textureLod(inputTexture,uv,0.).rgb;vec3 ssgiClrBright=pow(max(ssgiClr,vec3(0.)),vec3(2.));ssgiClr+=ssgiClrBright;switch(toneMapping){case 1:ssgiClr=LinearToneMapping(ssgiClr);break;case 2:ssgiClr=ReinhardToneMapping(ssgiClr);break;case 3:ssgiClr=OptimizedCineonToneMapping(ssgiClr);break;case 4:ssgiClr=ACESFilmicToneMapping(ssgiClr);break;case 5:ssgiClr=CustomToneMapping(ssgiClr);break;}ssgiClr*=toneMappingExposure;}outputColor=vec4(ssgiClr,1.0);}"; // eslint-disable-line
...
class SSAOEffect extends AOEffect {
  constructor(composer, camera, scene, options = {}) {
    SSAOEffect.DefaultOptions = { ...AOEffect.DefaultOptions,
      ...{
        spp: 16,
        distance: 1,
        distancePower: 0.25,
    ...
    options = { ...SSAOEffect.DefaultOptions,
      ...options
    };
